...
closestFoodPlantID field feederLoc = trace ("closest food plant id: " ++ (show plantID)) plantID
  where
    foodPlants = Map.filter (\plant -> case plant of
                                         Plant (Food amount) location -> True
                                         _                            -> False) (fieldPlants field)
    plantID = closestPlantID foodPlants feederLoc
...
moveTowardsLocation targetLocation seconds feeder = feeder { feederLocation=oldLocation `moveBy` movement }
  where
    oldLocation = trace ("moving towards " ++ (show targetLocation)) feederLocation feeder
    speedPerSecond = 1.8
...
iterateFeeder previousWorld fieldSoFar feeder seconds = let
    affectedFeeder = trace "ITERATING FEEDER" foldr (\effect feederAccum -> effect previousWorld feederAccum seconds) feeder lifeEffects
...
generatePossibleBehaviors previousWorld feeder = trace "GENERATING POSSIBLE BEHAVIORS" map ($ (previousWorld, feeder)) desires 
...
eat previousWorld fieldSoFar feeder seconds = let
    foodEatenPerSecond = trace "NOW EATING" 0.25
    ...
    p = (targetPlant fieldWithEatenPlant feeder)
    superField = trace (show p) fieldWithEatenPlant
  in
    (feeder { feederFood = clamp 0.0 1.0 ((feederFood feeder) + foodEaten) }, superField) --fieldWithEatenPlant)

drink :: Action
drink previousWorld fieldSoFar feeder seconds = let
    waterDrankPerSecond = trace "NOW DRINKING" 0.35
